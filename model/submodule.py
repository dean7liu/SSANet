...
def norm_correlation(fea1, fea2, fea1_sft, fea2_sft):
    cost = torch.mean((((fea1/(torch.norm(fea1, 2, 1, True)+1e-05)) * (fea2/(torch.norm(fea2, 2, 1, True)+1e-05))) - torch.abs(fea1_sft - fea2_sft)), dim=1, keepdim=True)
    return cost

def build_norm_correlation_volume(refimg_fea, targetimg_fea, refimg_fea_shift, targetimg_fea_shift, maxdisp):
    B, C, H, W = refimg_fea.shape
    volume = refimg_fea.new_zeros([B, 1, maxdisp, H, W])
    for i in range(maxdisp):
        if i > 0:
            volume[:, :, i, :, i:] = norm_correlation(refimg_fea[:, :, :, i:], targetimg_fea[:, :, :, :-i], refimg_fea_shift[:, :, :, i:], targetimg_fea_shift[:, :, :, :-i])
        else:
            volume[:, :, i, :, :] = norm_correlation(refimg_fea, targetimg_fea, refimg_fea_shift, targetimg_fea_shift)
    volume = volume.contiguous()
    return volume
