...
parser = argparse.ArgumentParser(description='Efficient and Accurate Stereo Matching via Semi-Sparsified Aggregation and Parallel-Distillation')
parser.add_argument('--model', default='SSANet', help='select a model structure:SSANet/SSANet_dist', choices=__models__.keys())
parser.add_argument('--maxdisp', type=int, default=192, help='maximum disparity')
parser.add_argument('--dataset', default='sceneflow', help='dataset name', choices=__datasets__.keys())
parser.add_argument('--datapath', default="/datapath/SceneFlow/", help='data path')
...
def train_sample(sample, compute_metrics=False):
    ...
    optimizer.zero_grad()
    disp_ests, err_mask = model(imgL, imgR)
    mask = (disp_gt < args.maxdisp) & (disp_gt > 0)
    mask_low = (disp_gt_low < args.maxdisp) & (disp_gt_low > 0)
    err_masks = err_mask > 0
    masks = [mask, mask_low, mask, mask_low]
    mask = mask & err_mask
    err_mask_d = [mask, mask_low, mask, mask_low]
    disp_gts = [disp_gt, disp_gt_low, disp_gt, disp_gt_low] 
    loss = model_loss_train(disp_ests, disp_gts, masks, err_mask_d)
